...
- [REQ-031] Child switcher: the Home, History, and Progress tabs must each display a persistent switcher banner (child name + age) at the top of the screen — not Home-only. When the family's Clerk org has more than one child profile, the banner is interactive: tapping it opens a bottom sheet to pick a different child, and swiping left/right anywhere on the screen cycles to the next/previous child (wrapping around at the ends) via a whole-screen card/page transition. Selecting or swiping to a different child updates the active child context across all three tabs. A family with one child still shows the banner (name + age) but with no interactive affordance (no tap, no swipe, no page dots). The banner does not appear on History Day-Detail or on Settings sub-screens. (revised 2026-07-01, see `.planning/phases/05-core-event-logging/05-CONTEXT.md` D-12 — supersedes the original Home-only, tap-only text)
...
| REQ-017 | Phase 3 | Complete |
| REQ-018 | Phase 3 | Complete |

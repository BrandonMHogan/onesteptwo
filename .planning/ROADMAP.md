...
- [x] **Phase 4: UI/UX Design** - Screen flows, lo-fi wireframes, design tokens (color, type, spacing), component specs, and navigation/animation patterns for all screens on both platforms — 13 findings (5 verification accuracy errors + 8 review warnings) found and fixed (gap closure completed 2026-06-30, see 04-REVIEW-FIX.md)
- [~] **Phase 5: Core Event Logging** - Onboarding wizard (family setup + consent + child profile), 4-tab navigation, one-tap logging with details-later flow, event types, pending-details banner, heatmap history tab, child switcher, Settings screens — Stage 1 complete (Android): theme, SQLDelight data layer, onboarding wizard, 4-tab shell, full Home tab Log/Toast/Sheet/Switcher loop, restyled Sign In, GET /v1/children endpoint. Stage 2 (History heatmap, Settings child/notification mgmt, iOS) not started — see 05-01-PLAN.md / 05-02-PLAN.md
...
**Wave 1**
  ...
  - [x] 02-04-PLAN.md — SQLDelight client schema: .sq current schema + 2.sqm migration for children, potty_events, consent_events with client-only sync_status (REQ-C-005)
...
**Wave 4** *(gap closure — from 02-VERIFICATION.md)*
  ...
  - [x] 02-06-PLAN.md — Account erasure completion: org-wide device_tokens delete via Clerk member list, Clerk Organization deletion after commit, org-traceable audit target_id, REQUIREMENTS.md traceability update (REQ-012, REQ-011, REQ-014, REQ-C-002/003/004/008) [Gaps 2 & 3]
...
**Wave 1**
  ...
  - [x] 04-02-PLAN.md — docs/SCREEN-FLOWS.md: auth/onboarding + main-app Mermaid navigation diagrams, log→toast→sheet interaction flow, platform navigation notes (REQ-035)
...
**Success Criteria** (what must be TRUE):
  ...
  3. A logged event can have event_type (pee / poo / both / accident / tried) and a text note appended after the initial tap; the event retains its original created_at timestamp regardless of when details are added
